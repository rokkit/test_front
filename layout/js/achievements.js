...
function bodyClick(e) {
	$('body').on('click', function (e) {
		...
		if ($("#watch_me_please").length) {
			$("#watch_me_please").css("z-index", "");
			$("#watch_me_please").css("pointer-events", "");
			$("#watch_me_please").removeAttr('id');
		}
...
/** Тут был очень страшный костыль! Давайте лучше отрисуем через drawAchievementDetail. */
function card(id) {
	var achiv = $(this);
	var el = $('#all_ach figure[data-id=' + id + ']');
	drawAchievementDetail(el.find('h6').text(), el.attr('data-description'), el.find('img').attr('src'), el.data('open'));
	fx.do([
		'achiv',
		'allAchivBG'
	]);
...
/** Core function to draw an achivement. */
function drawAchievementDetail(name, description, image, open) {
	/** Uhhh... Ensure that dimestions are right. */
	$("#achivka").parent().css("z-index", "9999");
	$("#achivka").parent().css("pointer-events", "all");
	$("#achivka").parent().attr("id", "watch_me_please");

	/** Ensure closing button. */
	$('#achivka .closing_item').attr("src", './images/cross.svg');
	$('#achivka .closing_item').show();
	$('#achivka .closing_item').off('click');
	$('#achivka .closing_item').on('click', function () {
		/** Backwards compatibility. */
		$("#achivka").parent().css("z-index", "");
		$("#achivka").parent().css("pointer-events", "");
		$("#achivka").parent().removeAttr('id');
		$(this).hide();
		//fx.back();
	});
...
$(function () {
  ...
  $.getJSON(window.hostUrl + '/api/v1/achievements.json', {
    ...
    $.each(json, function (i) {
      ...
      var template = '<figure data-open=' + this.open + ' data-id=' + this.id + ' onclick=\'card(' + this.id + ')\' data-description=\'' + this.description + '\'><img class=\'achievments_icon ' + klass + '\' src=\'' + window.hostUrl + this.image + '\'><figcaption><h6>' + this.name + '</h6><p>' + state + '</p></figcaption></figure>';
    ...
    $.each(json, function (i) {
      ...
      var template = '<figure data-open=' + this.open + ' data-description=\'' + this.description + '\'><img class=\'achievments_icon ' + klass + '\' src=\'' + window.hostUrl + this.image + '\'><figcaption><h6>' + this.name + '</h6><p>' + state + '</p></figcaption></figure>';
  ...
  $.getJSON(window.hostUrl + '/api/v1/skills.json', {
    ...
    $.each(json, function (i) {
      ...
      var template = '<figure data-cost=' + this.cost + ' data-name=\'' + this.name + '\' data-used_at=' + this.used_at + ' data-cooldown_end_at=\'' + this.cooldown_end_at + '\' data-can_take=' + this.can_take + ' data-has=' + this.has + ' data-id=' + this.id + ' data-description=\'' + this.description + '\'><img class=\'achievments_icon ' + klass + '\' src=\'' + window.hostUrl + this.image + '\'><figcaption><h6>' + this.name + '</h6><p>' + state + '</p></figcaption></figure>';
