...
Nodes = function () {
  ...
  function Nodes(el, w, h, nodes, links) {
    ...
    this.force = d3.layout.force().size([
      this.w,
      this.h
    ]).nodes(this.nodes).links(this.links).linkStrength(5).linkDistance(function (link) {
      var x, y;
  ...
  Nodes.prototype.render = function () {
    ...
    this.node.enter().insert('circle', '.cursor').attr('class', this.element + '-node').attr('fill', '#282C34').attr('data-id', function (node) {
      return node.id;
...
$(function () {
  ...
  this.nodesRight = new Nodes('viewport-right', w, h, Graph.nodes2, Graph.links2);  // this.nodesRight.render;
});