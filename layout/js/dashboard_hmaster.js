...
$(function () {
  ...
  var layout = d3.select('#face').append('svg').attr('width', layoutWidth).attr('height', layoutHeight);
  layout.append('defs').append('pattern').attr('id', 'face-img').attr('x', 0).attr('y', 0).attr('width', 200).attr('height', 200).attr('patternUnits', 'userSpaceOnUse').append('image').attr('x', 20).attr('y', 0).attr('width', 200).attr('height', 200);
  layout.append('circle').attr('r', 70).attr('fill', 'transparent').attr('stroke', '#fff').attr('opacity', 0.4).attr('cx', layoutWidth / 2).attr('cy', layoutHeight / 2);
  layout.append('circle').attr('r', 50).attr('stroke', '#EBB22F').attr('stroke-width', 2.5).attr('fill', 'transparent').attr('cx', layoutWidth / 2).attr('cy', layoutHeight / 2);
  layout.append('circle').attr('r', 44)  // .attr('stroke', '#EBB22F')
                                         // .attr('stroke-width', 2.5)
.attr('fill', 'url(#face-img)').attr('cx', layoutWidth / 2).attr('cy', layoutHeight / 2);
  for (var i = 0; i < 360; i += 12) {
    var r = rndR(68, 100);
    layout.append('line').attr('stroke', '#EBB22F').attr('opacity', function () {
      var res = 1;
      if (r < 85) {
        res = 0.6;
      }
      ...
      return res;
    }).attr('stroke-width', '2.4').attr('x1', function (d) {
...
function makeUserRating(users_month, users_all_time) {
  users_month = _.sortBy(_.filter(users_month, function (u) {
    return u.exp > 0;
  }), function (u) {
    u.exp;
  }).reverse();
  ...
  users_all_time = _.sortBy(_.filter(users_all_time, function (u) {
    return u.exp > 0;
  }), function (u) {
    u.exp;
  }).reverse();
